...
% the three key parameters of sine waves:
f = 2; % frequency 
A = 3; % amplitude 
phi = 0; % phase shift vector (in radians)
...
% try playing with the three key parameters of a sine wave (manipulate one
% at the time).
...
%% let's add noise (dsp version)
% skip if dsp is not installed in your version
...
%% let's add noise (randn version)

% generate Gaussian noise (base MATLAB)
noiseScales = [0 0.5 1.5];
wavesN = nan(length(noiseScales), N);
for wIdx = 1:length(noiseScales)
    noiseOut = randn(1, N);
    wavesN(wIdx,:) = waves(end,:) + noiseScales(wIdx) * noiseOut;
end
...
for wIdx = 1:size(wavesN,1)
    ...
    ylabel('\muV')
    ylim(max(abs(wavesN(:)))*[-1 1]);
    title(['noise scale = ' num2str(noiseScales(wIdx))])
    set(gca,'FontSize',12)
end
